      ...
      if (fileArg.includes('AdminConsoleView.tsx')) {
        ...
        if (tc.name === 'multi_replace_file_content') {
          ...
          if (typeof rawChunks === 'string') {
            try {
              // Escape literal control chars like newlines
              ...
              // Fallback to eval by wrapping in parentheses
              chunks = eval('(' + rawChunks + ')');
            }
          } else {
            chunks = rawChunks;
          }
        ...
        for (const chunk of chunks) {
          ...
          if (!content.includes(target)) {
            ...
            console.error('Target snippet (first 150 chars):\n', target.substring(0, 150));
            process.exit(1);
          }
